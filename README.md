...
As described previously, the implicit method is building a set of equations which then needs to be solved. The formula needed to do this can be derived if the derivatives are calculated numerically and is described below (Hull, 2015). A similar formula can also be found in Brennan (1977).

$$a_jf_{i,j-1}+b_jf_{i,j}+c_jf_{i,j+1}=\ f_{i+1,j}$$

where 

$$a_j=\frac{1}{2}\left(r-q\right)j∆t-12σ2j2∆t$$

$$b_j=1+\sigma^2j^2∆t+r∆t$$

$$c_j=-\frac{1}{2}\left(r-q\right)j∆t-12σ2j2∆t$$