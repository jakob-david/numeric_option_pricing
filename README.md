...
Figure 8 shows the results of the explicit and implicit finite difference methods. In contrast to the tree methods the finite dirrefence methods need to evaluate multiple stock prices in order to work. This is because in order to make an approximation of the derivative of the stock price one needs multiple stock prices. As can be seen immediately, the explicit method not only fails to converge but actually diverges when the time step is not sufficiently small, as is well known. In contrast, the implicit method demonstrates good convergence toward the correct solution.
